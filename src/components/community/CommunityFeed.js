...
function CommunityFeed() {
  ...
  const [allPosts, setAllPostsData] = useState([]);
  ...
  const isMyPostsRoute = location.pathname.includes('/my-posts');
  const postType = isMyPostsRoute ? 'myPosts' : 'allPosts';

  useEffect(() => {
    if (isMyPostsRoute && currentPage !== 1) {
      setCurrentPage(1);
      return;
    }
    if (!isMyPostsRoute && currentPage !== 1 && !location.pathname.includes('/community-posts')) {
      setCurrentPage(1);
      return;
    }
      ...
      if (isMyPostsRoute) {
        try {
          const myPostsResponse = await axios.get(
            `/account/my-posts?userId=${userId}&keyword=${debouncedSearchQuery}`
          );
          setMyPostsData(myPostsResponse.data);
        } catch (e) {
        ...
        try {
          const response = await axios.get(
            `/account/community-posts?page=${currentPage}&keyword=${debouncedSearchQuery}`
          );
          const { allPosts: fetchedAllPosts, numberOfDocs, limit } = response.data.allData;
          ...
          if (debouncedSearchQuery) {
            ...
            const myPostsResponse = await axios.get(
              `/account/my-posts?userId=${userId}`
            );
            setMyPostsData(myPostsResponse.data);
    ...
    userId,
    refreshTriggers.communityPosts,
    location.pathname,
    isMyPostsRoute
  ]);
      ...
      >
        ...
        <div className="p-5 flex-1 flex flex-col">
          <div className="flex justify-between items-start mb-2">
            ...
            <LikeUnlikePost className="like-button" post={post} />
          </div>
    ...
    cards = myPostsData.length ? (
      ...
      <div className="col-span-full text-center py10">
        <h2 className="text-xl text-gray-600">
          {hasSearchQuery ? "No posts found matching your search!" : "You haven't posted yet!"}
        </h2>
      </div>
    );
  } else {
    const displayData = hasSearchQuery ? searchedData : allPosts;

    cards = displayData.length ? (
      displayData.map((post) => buildCard(post))
    ) : (
      <div className="col-span-full text-center py10">
        <h2 className="text-xl text-gray-600">
          {hasSearchQuery ? "No posts found!" : "No community posts yet!"}
        </h2>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 space-y-4 md:space-y-0">
        ...
        <div className="w-full md:w-auto">
          <SearchPosts searchValue={searchQuery} onSearchChange={setSearchQuery} />
        </div>
      </div>
