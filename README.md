...
Step 1: Clone this repository. => `cd PetOpia` ; Run `npm install` in the `PetOpia` folder

Step 2: Run `npm start` in the PetOpia directory to start the frontend server of PetOpia

Step 53 Navigate to the route [http://localhost:3000]() (default) or whatever URL is used by the client (React) to start the server to get started with using PetOpia
